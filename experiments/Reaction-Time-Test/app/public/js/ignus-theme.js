...
ignusTheme.hsControlRequirement = function () {
    ...
    for (k = 0; k < hsElements.length; k++) {
        ...
        if (hsElement.scrollLeft == 0) {
            // use classes to hide the left button,
            // also handle padding
        ...
        if (Math.abs((hsElement.scrollLeft + hsElement.clientWidth) - hsElement.scrollWidth) <= 1) {
            // use classes to hide the right button
            if (hsRight.classList.contains('d-sm-flex')) {
                hsRight.classList.remove('d-sm-flex')
            }
        }
